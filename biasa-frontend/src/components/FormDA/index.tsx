...
function ST({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 11.5, fontWeight: 600, color: '#8a96a3', letterSpacing: '.5px', paddingBottom: 8, borderBottom: '1px solid rgba(0,0,0,0.08)', marginBottom: 12, marginTop: 16, textTransform: 'uppercase' as const }}>{children}</div>
}
function FL({ label, children, style, required }: { label: string; children: React.ReactNode; style?: React.CSSProperties; required?: boolean }) {
  return <div style={style}><label style={{ display: 'block', fontSize: 12.5, color: '#5e6f85', marginBottom: 4 }}>{label}{required && <span style={{ color: '#c0392b', marginLeft: 2 }}>*</span>}</label>{children}</div>
}

export default function FormDA({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const ms: React.CSSProperties = { background: '#fff', borderRadius: 12, border: '1px solid rgba(0,0,0,0.10)', width: '100%', maxWidth: etape === 'confirm' ? 420 : 640, display: 'flex', flexDirection: 'column', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }
  ...
  const inpStyle: React.CSSProperties = { fontSize: 13.5, padding: '8px 10px', border: '1px solid rgba(0,0,0,0.12)', borderRadius: 6, background: '#eaf2fb', color: '#0B3C7A', width: '100%', outline: 'none', fontFamily: 'inherit' }
        ...
        <div style={ms}>
          <div style={{ padding: '16px 18px', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ fontSize: 12.5, color: '#5e6f85', marginTop: 2 }}>Votre responsable sera notifié par email</div>
            </div>
            <button onClick={() => setEtape('form')} style={{ width: 28, height: 28, border: '1px solid rgba(0,0,0,0.12)', borderRadius: 6, background: 'transparent', cursor: 'pointer', fontSize: 17.5, color: '#5e6f85' }}>×</button>
          </div>
            ...
            <div style={{ border: '1px solid rgba(0,0,0,0.08)', borderRadius: 8, overflow: 'hidden' }}>
              {[
                ...
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 14px', borderBottom: '1px solid rgba(0,0,0,0.06)', fontSize: 13.5 }}>
                  <span style={{ color: '#5e6f85' }}>{l}</span>
          ...
          <div style={foot}>
            <button onClick={() => setEtape('form')} style={{ padding: '8px 16px', fontSize: 13.5, border: '1px solid rgba(0,0,0,0.12)', borderRadius: 6, background: 'transparent', cursor: 'pointer', color: '#5e6f85' }}>← Retour</button>
        ...
        <div style={ms}>
          <div style={{ padding: '16px 18px', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ fontSize: 12.5, color: '#5e6f85', marginTop: 2 }}>Le service et au moins un article (désignation + quantité) sont requis ; le reste est facultatif</div>
            </div>
            <button onClick={onClose} style={{ width: 28, height: 28, border: '1px solid rgba(0,0,0,0.12)', borderRadius: 6, background: 'transparent', cursor: 'pointer', fontSize: 17.5, color: '#5e6f85' }}>×</button>
          </div>
          ...
          <div style={{ padding: 16, maxHeight: '75vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 4 }}>
              {(['medical', 'bien_service'] as TypeDA[]).map(t => (
                <button key={t} onClick={() => set('type_da', t)} style={{ flex: 1, padding: 10, border: form.type_da === t ? '2px solid #0B3C7A' : '1px solid rgba(0,0,0,0.12)', borderRadius: 8, cursor: 'pointer', fontSize: 13.5, background: form.type_da === t ? '#1B9DE0' : '#eaf2fb', color: form.type_da === t ? '#fff' : '#5e6f85', fontWeight: form.type_da === t ? 600 : 400 }}>
            ...
            <div style={{ marginBottom: 4 }}>
              <div style={{ fontSize: 12.5, color: '#5e6f85', marginBottom: 6 }}>Motif <span style={{ color: '#c0392b' }}>*</span></div>
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' as const, marginBottom: 10 }}>
                {([['reappro','Réapprovisionnement'],['nouveau_besoin','Nouveau besoin'],['commande_specifique','Cmd. spécifique'],['remplacement','Remplacement'],['activite_urgente','Activité urgente']] as [MotifDA,string][]).map(([v,l]) => (
                  <button key={v} onClick={() => set('motif', v)} style={{ padding: '5px 10px', border: form.motif === v ? '2px solid #0B3C7A' : '1px solid rgba(0,0,0,0.12)', borderRadius: 20, cursor: 'pointer', fontSize: 12.5, background: form.motif === v ? '#1B9DE0' : '#eaf2fb', color: form.motif === v ? '#fff' : '#5e6f85', fontWeight: form.motif === v ? 600 : 400 }}>{l}</button>
                ))}
              </div>
              <div style={{ fontSize: 12.5, color: '#5e6f85', marginBottom: 6 }}>Urgence <span style={{ color: '#c0392b' }}>*</span></div>
              <div style={{ display: 'flex', gap: 5, marginBottom: 10 }}>
                {([['haute','Haute — 24h'],['moyenne','Moyenne — 48h'],['faible','Faible — >72h']] as [UrgenceDA,string][]).map(([v,l]) => (
                  <button key={v} onClick={() => set('urgence', v)} style={{ flex: 1, padding: '7px', border: form.urgence === v ? '2px solid #0B3C7A' : '1px solid rgba(0,0,0,0.12)', borderRadius: 8, cursor: 'pointer', fontSize: 12.5, background: form.urgence === v ? '#1B9DE0' : '#eaf2fb', color: form.urgence === v ? '#fff' : '#5e6f85', fontWeight: form.urgence === v ? 600 : 400 }}>{l}</button>
                ))}
              </div>
                ...
                <thead>
                  <tr style={{ background: '#eaf2fb' }}>
                    {['N°','Désignation *','Qté *','Unité',...(isMed ? ['Stock actuel','Réf. / Marque'] : ['Description technique']),'Obs.'].map(h => (
                      <th key={h} style={{ fontSize: 11.5, fontWeight: 600, color: '#5e6f85', padding: '7px 6px', borderBottom: '1px solid rgba(0,0,0,0.08)', textAlign: 'left', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {form.lignes.map((l, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                      <td style={mtd}><span style={{ fontSize: 12.5, color: '#8a96a3', padding: '0 4px' }}>{l.numero_ligne}</span></td>
            ...
            <div onClick={() => fileInputRef.current?.click()} style={{ border: '1px dashed rgba(0,0,0,0.15)', borderRadius: 8, padding: '16px', textAlign: 'center', cursor: 'pointer', marginBottom: 10, background: '#fafafa' }}>
              <div style={{ fontSize: 13.5, color: '#5e6f85' }}>Cliquer pour ajouter un fichier</div>
              <div style={{ fontSize: 11.5, color: '#8a96a3', marginTop: 3 }}>PDF, Word, Excel, Image · Max 10 Mo</div>
            </div>
            ...
            {fichiers.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {fichiers.map((f, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 6, background: '#eaf2fb' }}>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 13.5 }}>{f.name}</div><div style={{ fontSize: 11.5, color: '#8a96a3' }}>{Math.round(f.size / 1024)} Ko</div></div>
                    <button onClick={() => supprimerFichier(i)} style={{ fontSize: 12.5, color: '#8a96a3', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px' }}>Retirer</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={foot}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={onClose} style={{ padding: '8px 16px', fontSize: 13.5, border: '1px solid rgba(0,0,0,0.12)', borderRadius: 6, background: 'transparent', cursor: 'pointer', color: '#5e6f85' }}>Annuler</button>
              <button
