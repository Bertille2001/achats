...
function Stat({ label, value, color = '#0B3C7A' }: { label: string; value: number; color?: string }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #e3e9f1', borderRadius: 8, padding: '14px 16px' }}>
      <div style={{ fontSize: 11.5, color: '#8a96a3', marginBottom: 6 }}>{label}</div>
...
export default function DashboardPage() {
  ...
  if (loading) return <div style={{ padding: 32, color: '#8a96a3', fontSize: 13.5 }}>Chargement…</div>
  ...
  return (
    <>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid #e3e9f1', background: '#fff' }}>
        <div style={{ fontSize: 16.5, fontWeight: 600, color: '#0B3C7A' }}>Bonjour, {utilisateur?.prenom}</div>
        <div style={{ fontSize: 12.5, color: '#8a96a3', marginTop: 2 }}>
      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        {role && ROLE_EXPLICATIONS[role] && (
          <div style={{ background: '#eaf2fb', border: '1px solid #e3e9f1', borderRadius: 8, padding: '12px 14px', marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10, marginBottom: 18 }}>
          {(role === 'demandeur' || role === 'admin') && <>
            ...
            <Stat label="En cours" value={enCours} color="#1B9DE0" />
            <Stat label="Approuvées" value={approuvees} color="#1B9DE0" />
            <Stat label="Reçues" value={recues} color="#0B3C7A" />
            <Stat label="Rejetées" value={rejetees} color="#a32d2d" />
          </>}
        ...
        <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          {/* Dernières demandes */}
          <div style={{ background: '#fff', border: '1px solid #e3e9f1', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ padding: '11px 14px', borderBottom: '1px solid #eef2f7', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {mesDa.length === 0
              ? <div style={{ padding: '20px 14px', fontSize: 13.5, color: '#8a96a3', textAlign: 'center' }}>Aucune demande</div>
              : mesDa.slice(0, 3).map(da => (
                <div key={da.id} onClick={() => navigate(`/demandes/${da.id}`)} style={{ padding: '9px 14px', borderBottom: '1px solid #eaf2fb', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11.5, color: '#8a96a3', marginTop: 1 }}>{fmt(da.date_demande)}</div>
                  </div>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12.5, color: '#5e6f85' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {role === 'demandeur' && (
              <div style={{ background: '#fff', border: '1px solid #e3e9f1', borderRadius: 8, padding: '14px' }}>
                ...
                <button onClick={() => navigate('/mes-demandes')} style={{ width: '100%', padding: '10px 12px', fontSize: 13.5, border: '1px solid #e3e9f1', borderRadius: 7, background: 'transparent', color: '#5e6f85', cursor: 'pointer', textAlign: 'left' as const }}>
            ...
            {(role === 'responsable' || role === 'daf' || role === 'acheteur') && (
              <div style={{ background: '#fff', border: '1px solid #e3e9f1', borderRadius: 8, overflow: 'hidden' }}>
                ...
                {aValider.length === 0
                  ? <div style={{ padding: '20px 14px', fontSize: 13.5, color: '#8a96a3', textAlign: 'center' }}>Aucune demande en attente</div>
                  : aValider.slice(0, 3).map(da => (
                    <div key={da.id} onClick={() => navigate(`/demandes/${da.id}`)} style={{ padding: '9px 14px', borderBottom: '1px solid #eaf2fb', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontSize: 13.5, fontWeight: 500, color: '#0B3C7A' }}>{da.numero}</div>
                        <div style={{ fontSize: 11.5, color: '#8a96a3', marginTop: 1 }}>{da.demandeur?.prenom} {da.demandeur?.nom} · {fmt(da.date_demande)}</div>
                      </div>
                      {da.urgence === 'haute' && (
              ...
              <div style={{ background: '#fff', border: '1px solid #e3e9f1', borderRadius: 8, overflow: 'hidden' }}>
                ...
                {aValider.slice(0, 3).map(da => (
                  <div key={da.id} onClick={() => navigate(`/demandes/${da.id}`)} style={{ padding: '9px 14px', borderBottom: '1px solid #eaf2fb', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: 13.5, fontWeight: 500, color: '#0B3C7A' }}>{da.numero}</div>
                      <div style={{ fontSize: 11.5, color: '#8a96a3', marginTop: 1 }}>{da.demandeur?.prenom} {da.demandeur?.nom}</div>
                    </div>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12.5, color: '#5e6f85' }}>
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: dotColor(da.statut), flexShrink: 0 }} />
                      {STATUT_LABELS[da.statut]}
