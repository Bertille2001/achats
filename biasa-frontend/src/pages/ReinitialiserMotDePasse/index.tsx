    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ background: 'var(--bg-primary)', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', padding: '32px 28px' }}>
          ...
          {succes ? (
            <div style={{ padding: '12px 14px', fontSize: 12.5, borderRadius: 8, color: '#0B3C7A', background: 'var(--bg-secondary)', border: '1px solid #dbe9f7' }}>
              Mot de passe mis à jour. Redirection vers la connexion…
            </div>
          ) : (
            <>
              <div style={{ fontSize: 13.5, color: 'var(--text-secondary)', marginBottom: 22 }}>Choisissez votre nouveau mot de passe.</div>
              {erreur && (
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: 14 }}>
                  <label style={{ display: 'block', fontSize: 11.5, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 6 }}>NOUVEAU MOT DE PASSE</label>
                  <input type="password" value={nouveau} onChange={e => setNouveau(e.target.value)} required style={inp} />
                </div>
                <div style={{ marginBottom: 24 }}>
                  <label style={{ display: 'block', fontSize: 11.5, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 6 }}>CONFIRMER</label>
